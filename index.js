...
app.get('/', (req, res) => res.send(`PAVE-BOT: ${botStatus} - /qr per QR`))
app.get('/qr', (req, res) => {
    if (!latestQR) return res.status(404).send('Nessun QR - bot gia connesso o in avvio')
...
async function startBot() {
    try {
        ...
        sock.ev.on('connection.update', async (update) => {
            ...
            if (qr) { latestQR = qr; botStatus = "QR PRONTO"; console.log(`QR PRONTO su /qr`); qrcode.generate(qr, { small: true }) }
            if (connection === 'close') {
                botStatus = "DISCONNESSO"
                const shouldReconnect = lastDisconnect?.error?.output?.statusCode!== 401
                if (shouldReconnect) { botStatus = "RICONNESSIONE..."; setTimeout(startBot, 3000) }
                else { botStatus = "SESSIONE SCADUTA" }
            } else if (connection === 'open') { botStatus = "✅ CONNESSO"; console.log(`✅ ${BOT_NAME} CONNESSO`) }
        })
