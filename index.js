require('fs').rmSync('./auth',{recursive:true,force:true}); // <-- TOGLI QUESTA RIGA DOPO COLLEGATO
...
async function startBot() {
    ...
    const sock = makeWASocket({ auth: state, logger: P({ level: 'silent' }), printQRInTerminal: false, browser: ["Holy Bot", "Chrome", "1.0"] })
    sock.ev.on('creds.update', saveCreds)

    if (!sock.authState.creds.registered) {
        setTimeout(async () => {
            try {
                let code = await sock.requestPairingCode("393381532143")
                console.log(`\n\n=== CODICE PAIRING PER 3381532143: ${code} ===\n\n`)
    ...
    sock.ev.on('connection.update', (u) => {
        ...
        if (u.connection === 'open') console.log("✅ BOT CONNESSO!")
    })
    ...
    sock.ev.on('messages.upsert', async ({ messages }) => {
        ...
        // CAR MENU
            ...
            let p=lower.split(' '); let code=p[1]; let price=parseInt(p[2]); if(!price) return sock.sendMessage(jid,{text:`Usa: /sell CODICE PREZZO`})
            let cars=garage[jid]||[]; let i=cars.findIndex(c=>c.code===code); if(i===-1) return sock.sendMessage(jid,{text:`Codice non trovato`})
            let car=cars[i]; cars.splice(i,1); saveGarage({...garage,[jid]:cars})
            let market=getMarket(); market.push({name:car.name,code:car.code,price:price,seller:jid,value:car.value||price}); saveMarket(market)
            await sock.sendMessage(jid,{text:`*🏷️ ${car.name} in vendita per ${price}€*`})
        }
            ...
            await sock.sendMessage(jid,{text:`*♻️ AutoSell ${car.name} +${half}€*`})
        }
            ...
            if(mc){
                if((moneyDb[jid]||0)<mc.price) return sock.sendMessage(jid,{text:`Soldi insufficienti`})
                moneyDb[jid]=(moneyDb[jid]||0)-mc.price; if(!garage[jid]) garage[jid]=[]; garage[jid].push({name:mc.name,code:genCode(),value:mc.value||mc.price}); saveMarket(market.filter(c=>c.code!==code)); saveMoney(moneyDb); saveGarage(garage);
                await sock.sendMessage(jid,{text:`✅ Comprata ${mc.name}`}); return
            }
            let sc=shop.find(c=>c.code===code)
            if(sc){
                if((moneyDb[jid]||0)<sc.price) return sock.sendMessage(jid,{text:`Soldi insufficienti`})
                moneyDb[jid]=(moneyDb[jid]||0)-sc.price; if(!garage[jid]) garage[jid]=[]; garage[jid].push({name:sc.name,code:genCode(),value:sc.price}); saveMoney(moneyDb); saveGarage(garage);
                await sock.sendMessage(jid,{text:`✅ Comprata ${sc.name}`}); return
            }
        }

        // SLOT MENU
        else if (lower === '/slotm') await sock.sendMessage(jid, { text: `*🎰Slot Menu🎰*\n/slotshop\n/myslot\n/slot\n/buyslot CODICE\n/sellslot CODICE PREZZO\n/autosellslot CODICE\n/slot CODICE (per giocare)\n/slot% CODICE PERC\n/slot$ CODICE PAYOUT` })
        else if (lower === '/slotshop') {
            let txt=`*🎰 SLOT SHOP 🎰*\n\n`; slotshop.forEach(s=>txt+=`• ${s.name} - ${s.price}€ Win ${s.win}% - /buyslot ${s.code}\n`)
            await sock.sendMessage(jid,{text:txt})
        }
        else if (lower === '/myslot') {
            let slots=getSlot()[jid]||[]; if(slots.length===0) return sock.sendMessage(jid,{text:`Slot vuote`})
            await sock.sendMessage(jid,{text:`*🎰 Your Slots*\n\n`+slots.map(s=>`• ${s.name} - ${s.code} Win ${s.win}% Payout ${s.payout}€`).join('\n')})
        }
        else if (lower === '/slot' || lower === '/slots') {
            let market=getSlotMarket(); if(market.length===0) return sock.sendMessage(jid,{text:`Slot Market vuoto`})
            let txt=`*🌐 Slot On Market 🌐*\n\n`; market.forEach(m=>txt+=`• ${m.name} - ${m.price}€ Win ${m.win}% - /buyslot ${m.code}\n`)
            await sock.sendMessage(jid,{text:txt})
        }
        else if (lower.startsWith('/slot ') &&!lower.includes('%') &&!lower.includes('$')) {
            let code=lower.split(' ')[1]; let mySlots=getSlot()[jid]||[]; let slot=mySlots.find(s=>s.code===code); if(!slot) return sock.sendMessage(jid,{text:`Slot non trovata`})
            let moneyDb=getMoney(); if(Math.random()*100<=slot.win){ moneyDb[jid]=(moneyDb[jid]||0)+slot.payout; saveMoney(moneyDb); await sock.sendMessage(jid,{text:`🎰 WIN +${slot.payout}€ su ${slot.name}!`}) } else await sock.sendMessage(jid,{text:`🎰 LOST su ${slot.name}`})
        }
        else if (lower.startsWith('/buyslot ')) {
            let code=lower.split(' ')[1]; let moneyDb=getMoney(); let market=getSlotMarket();
            let ms=market.find(s=>s.code===code)
            if(ms){
                if((moneyDb[jid]||0)<ms.price) return sock.sendMessage(jid,{text:`Soldi insufficienti`})
                moneyDb[jid]-=ms.price; let all=getSlot(); if(!all[jid]) all[jid]=[]; all[jid].push({name:ms.name,code:genCode(),win:ms.win,payout:ms.payout,value:ms.price}); saveSlot(all); saveMoney(moneyDb); saveSlotMarket(market.filter(s=>s.code!==code));
                await sock.sendMessage(jid,{text:`✅ Slot ${ms.name} comprata`}); return
            }
            let sh=slotshop.find(s=>s.code===code); if(!sh) return;
            if((moneyDb[jid]||0)<sh.price) return sock.sendMessage(jid,{text:`Soldi insufficienti`})
            moneyDb[jid]-=sh.price; let all=getSlot(); if(!all[jid]) all[jid]=[]; all[jid].push({name:sh.name,code:genCode(),win:sh.win,payout:Math.floor(sh.price*0.3),value:sh.price}); saveSlot(all); saveMoney(moneyDb);
            await sock.sendMessage(jid,{text:`✅ Slot ${sh.name} comprata`})
        }
            ...
            await sock.sendMessage(jid,{text:`Slot ${s.name} in vendita per ${price}€`})
        }
            ...
            await sock.sendMessage(jid,{text:`♻️ AutoSell Slot +${half}€`})
        }
        else if (lower.startsWith('/slot% ')) {
            let p=lower.split(' '); let code=p[1]; let perc=parseInt(p[2]); let all=getSlot(); for(let u in all){ let idx=all[u].findIndex(s=>s.code===code); if(idx!==-1){ all[u][idx].win=perc; saveSlot(all); await sock.sendMessage(jid,{text:`Win ${perc}% impostato`}); return } }
        }
        else if (lower.startsWith('/slot$ ')) {
            let p=lower.split(' '); let code=p[1]; let pay=parseInt(p[2]); let all=getSlot(); for(let u in all){ let idx=all[u].findIndex(s=>s.code===code); if(idx!==-1){ all[u][idx].payout=pay; saveSlot(all); await sock.sendMessage(jid,{text:`Payout ${pay}€ impostato`}); return } }
        }
