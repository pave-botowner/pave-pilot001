...
async function startBot() {
    try {
        ...
        sock.ev.on('connection.update', async (update) => {
            ...
            } else if (connection === 'open') { latestQR = null; console.log(`✅ ${BOT_NAME} CONNESSO`) }
        })
        sock.ev.on('messages.upsert', async ({ messages }) => {
            ...
            else if (lower.startsWith("/slot")) { let amount = parseInt(lower.split(" ")[1]) || 100; if (isNaN(amount) || amount <= 0) return sock.sendMessage(chatJid, { text: `Usa: /slot 100` }); if (amount < 10) return sock.sendMessage(chatJid, { text: `Minimo 10€` }); if (db[senderJid].soldi < amount) return sock.sendMessage(chatJid, { text: `Non hai ${amount}€, hai ${db[senderJid].soldi}€` }); const symbols = ["🍒","🍋","🍊","🔔","💎","7️⃣"]; const r1 = symbols[Math.floor(Math.random()*symbols.length)]; const r2 = symbols[Math.floor(Math.random()*symbols.length)]; const r3 = symbols[Math.floor(Math.random()*symbols.length)]; let win = 0, msg = ""; if (r1===r2 && r2===r3) { win = amount * 5; msg = `JACKPOT! 🎉 5x` } else if (r1===r2 || r2===r3 || r1===r3) { win = amount * 2; msg = `Vinto! 2x` } else { win = 0; msg = `Perso!` } db[senderJid].soldi -= amount; db[senderJid].soldi += win; saveDB(db); return sock.sendMessage(chatJid, { text: `*🎰 SLOT ${amount}€*\n| ${r1} | ${r2} | ${r3} |\n${msg}\n${win>0?`Vinto ${win}€`:`Perso ${amount}€`}\nSaldo: ${db[senderJid].soldi}€` }) }
