...
console.log("RESET AUTH FATTO - AVVIO BOT...")
...
async function startBot() {
    ...
    console.log("CHIEDO CODICE PAIRING PER 393381532143...")
    setTimeout(async () => {
        if (!sock.authState.creds.registered) {
            try {
                let code = await sock.requestPairingCode("393381532143")
                console.log(`\n\n=== CODICE: ${code} ===\n INSERISCI SU WHATSAPP > DISPOSITIVI COLLEGATI > COLLEGA CON NUMERO\n\n`)
            } catch(e){ console.log("ERRORE CODICE:", e.message) }
        } else {
    ...
    sock.ev.on('connection.update', (u) => {
        ...
        if (u.connection === 'open') console.log("✅ BOT CONNESSO! PUOI TOGLIERE LA RIGA rmSync")
    })
